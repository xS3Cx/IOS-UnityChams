                               ...
                               int maxMagnitudeDelta)
{
    ...
    if (magAxis == 0)
        axis = Normalized(Cross(current, current + VInt3(static_cast<int>(3.95), static_cast<int>(5.32), static_cast<int>(-4.24))));
...
char VInt3::ToChar(VInt3 a) {
    const char* x = reinterpret_cast<const char*>(a.X);
    const char* y = reinterpret_cast<const char*>(a.Y);
    const char* z = reinterpret_cast<const char*>(a.Z);
